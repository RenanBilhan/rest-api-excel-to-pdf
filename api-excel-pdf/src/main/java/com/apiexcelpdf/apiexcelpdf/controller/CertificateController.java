...
public class CertificateController {
    ...
    public String generatePdf(@RequestParam @Valid MultipartFile excel) throws IOException, JRException {
        String retorno = "";
        List<CertificateDto> listDto = new ArrayList<>(certificateService.create(excel));
        List<CertificateDto> singleDto = new ArrayList<>();
        for(int i = 0; i< listDto.size(); i++ ) {
            singleDto.add(listDto.get(i));
            JRBeanCollectionDataSource beanCollectionDataSource = new JRBeanCollectionDataSource(singleDto);
